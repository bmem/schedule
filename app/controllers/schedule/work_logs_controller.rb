module Schedule
  class WorkLogsController < Schedule::ApplicationController
    # GET /work_logs
    # GET /work_logs.json
    def index
      ex = select_options
      ...
      @work_logs = @work_logs.where(:person_id => ex.person) if ex.person
    ...
    def select_options(work = WorkLog.new)
      ...
      work.person ||= readable(Person.find(params[:person_id])) if params[:person_id]
      @event ||= work.shift.event if work.shift
      work.event ||= @event
      @positions = @person ? @person.positions : readable(Position).order(:name)
      # TODO create a way to associate people with an event
      @people = @person ? [@person] : readable(Person).order(:name)
